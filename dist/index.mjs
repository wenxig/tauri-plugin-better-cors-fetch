...
var CORSFetch = class CORSFetch {
  ...
  static setCookie(url, content) {
    return invoke('plugin:cors-fetch|set_cookie', { config: { url: String(url), content } })
  }
  static getCookie(url, name) {
    return invoke('plugin:cors-fetch|get_cookie', { config: { url: String(url), name } })
  }
  ...
  static deleteCookie(url, name) {
    return invoke('plugin:cors-fetch|delete_cookie', { config: { url: String(url), name } })
  }
  ...
  _streamConfig = { bufferSize: 5, maxBufferBytes: 256 * 1024 }
  _config = {
    include: [],
    exclude: [],
    request: {
  ...
  async fetch(input, init, force = false) {
    ...
    try {
      ...
      const {
        ...
        rid: _rid
      } = await invoke('plugin:cors-fetch|fetch_send', { rid })
      responseRid = _rid
      if (signal?.aborted) throw this.cancel_error
      const chunkBuffer = []
